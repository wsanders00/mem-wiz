...
def build_status_payload(config) -> dict[str, Any]:
    workspace_inbox = list_workspace_records(config, "inbox")
    workspace_canon = list_workspace_records(config, "canon")
    workspace_archive = list_workspace_records(config, "archive")
    global_canon = list_global_records(config, "canon")
    ...
    return {
        ...
        "counts": {
            "workspace_inbox": len(workspace_inbox),
            "workspace_canon": len(workspace_canon),
            "workspace_archive": len(workspace_archive),
            "global_canon": len(global_canon),
...
def _latest_record_timestamp(paths: list[Path]) -> str | None:
    timestamps = [
        read_record(path).updated_at
        for path in paths
    ]
